...
const WAITLIST_GUILD_ID = process.env.DISCORD_GUILD_ID!;
...
export async function startDiscordBot() {
  ...
  client.on(Events.GuildMemberAdd, async (member) => {
    if (WAITLIST_GUILD_ID && member.guild.id !== WAITLIST_GUILD_ID) return;
      ...
      try {
        ...
        if (data.linked) {
          const embed = new EmbedBuilder()
            ...
            .setDescription("Your Discord is connected to the waitlist")
            .addFields(
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              "Your Discord isn't connected yet!\n\n" +
              "Visit **https://waitlist.example.com/dashboard** to connect and earn **+25 bonus points**."
            )
    ...
    if (interaction.commandName === "leaderboard") {
      ...
      try {
        ...
        const embed = new EmbedBuilder()
          ...
          .setFooter({ text: "waitlist.example.com" });
    ...
    if (interaction.commandName === "rank") {
      ...
      try {
        ...
        if (data.linked) {
          ...
          await interaction.editReply({
            content: "🔗 Link your account at **https://waitlist.example.com/dashboard**",
          });
        }
      } catch (error) {
      ...
      try {
        ...
        if (data.linked && data.referralCode) {
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              `**https://waitlist.example.com?ref=${data.referralCode}**\n\n` +
              "• Click: +1 pt (max 100)\n• Signup: +5 pts\n• Activated: +10 pts"
            )
            .addFields({
              name: "Your Stats",
          ...
          await interaction.editReply({
            content: "🔗 Link your account first at **https://waitlist.example.com/dashboard**",
          });
        }
      } catch (error) {
